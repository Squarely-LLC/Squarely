...
const saveExternalQuotation = async () => {
  ...
  const updatedQuotation = cloneQuotationRecord(currentQuotation);
  ...
  updatedQuotation.quotation.service = "Imported quotation";
  ...
  updatedQuotation.quotation.balance =
    getQuotationOutstandingBalance(updatedQuotation);
  updatedQuotation.paymentDetails.totalDue = formatCurrencyAmount(
    updatedQuotation.quotation.balance,
    configStore.financial,
  );

  const savedQuotation = quotationsStore.updateQuotation(
  ...
  notifications.push(
    `Quotation ${savedQuotation.quotation.quoteNumber} updated successfully.`,
    "success",
    3500,
  );
  clearQuotationPreviewDraft();
...
<template>
  <VRow v-if="quotationData?.quotation && isExternalQuotation">
    <VCol cols="12" md="8">
      <VCard>
        <VCardItem>
          <VCardTitle>Edit Imported Quotation</VCardTitle>
          <VCardSubtitle>
