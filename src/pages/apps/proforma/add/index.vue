...
const buildBlankQuotation = (): ProformaData => {
  ...
  return {
    quotation: {
      id,
      quoteNumber: `${getDocumentSequencePrefix("proforma")}${id}`,
    ...
    note: buildProformaNote(configStore.financial, 7),
...
const buildRevisionDraft = (parentId: number): ProformaData => {
  const sourceRecord = proformasStore.byId(parentId);
  ...
  const maxExistingNumber = proformasStore.all.reduce((max, record) => {
...
const buildDuplicateDraft = (sourceId: number): ProformaData => {
  const sourceRecord = proformasStore.byId(sourceId);
  if (!sourceRecord) return buildBlankQuotation();

  const duplicateId = proformasStore.nextId();
  const duplicateDraft = cloneProformaRecord(sourceRecord);
  const total = getQuotationGrandTotal(duplicateDraft.purchasedProducts);

  duplicateDraft.quotation.id = duplicateId;
  duplicateDraft.quotation.quoteNumber = `${getDocumentSequencePrefix("proforma")}${duplicateId}`;
  ...
  duplicateDraft.convertedInvoiceId = null;
...
const buildInitialQuotation = (): ProformaData => {
...
const quotationData = ref<ProformaData>(buildInitialQuotation());
...
const buildDraftSnapshot = (draft: ProformaData) =>
  JSON.stringify(cloneProformaRecord(draft));
...
const openPreview = async () => {
  ...
  const previewDraft = cloneProformaRecord(quotationData.value);
  ...
  previewDraft.quotation.balance = getProformaOutstandingBalance(previewDraft);
      ...
      <VCard class="mb-8">
        <VCardText>
          ...
          <VBtn
            ...
            variant="tonal"
            class="mb-4"
            @click="requestLeave({ name: 'apps-proforma-list' })"
            >Back to List</VBtn
          >
  ...
  <VDialog v-model="isLeaveDialogVisible" max-width="440" persistent>
    <VCard>
      <VCardText class="pt-6">
        You have unsaved changes in this proforma. Leave this page without
        saving?
      </VCardText>
