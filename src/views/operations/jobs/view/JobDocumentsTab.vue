...
const docsList = computed<JobDocument[]>(() => {
  // prefer the authoritative source from the jobs store for the
  // job's documents so UI updates immediately after store mutations.
...
function removeDocument(d: JobDocument) {
  ...
  const existing = (job.value.documents || []).filter((x) => x.id !== d.id);
  jobsStore.updateJob(job.value.id, { documents: existing });
...
function onDialogSave(payload: JobDocument) {
  ...
  if (!job.value) {
    try {
      // eslint-disable-next-line no-console
      console.warn("JobDocumentsTab.onDialogSave: no job.value, aborting");
    } catch {}
    return;
  }

  const existing = Array.isArray(job.value.documents)
    ? [...job.value.documents]
    : [];
  ...
  try {
    // eslint-disable-next-line no-console
    console.log("JobDocumentsTab: updated docs", existing.length, existing);
  } catch {}
  dialogOpen.value = false;
}

// reset localDocs when job changes
watch(
