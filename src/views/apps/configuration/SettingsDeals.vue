...
const sanitizeCustomField = (
  ...
  const key = normalizeFieldKey(field.key || field.label || `field_${Date.now()}`)
  const label = formatEntry(field.label || field.key || 'Field')
  const options = type === 'select' ? formatFieldOptions(field.options) : undefined
...
const loadData = () => {
  ...
  customFields.value = configuredCustomFields.map(field => sanitizeCustomField(field))
}
...
const updateCustomField = (
  ...
  if (patch.label)
    next[index].key = normalizeFieldKey(patch.label)
...
const removeCustomField = (index: number) => {
  customFields.value = customFields.value.filter((_, fieldIndex) => fieldIndex !== index)
}
</script>
        ...
        <VRow>
          <VCol cols="12" md="6">
            <AppTextField
              ...
              @update:model-value="updateCustomField(index, { label: String($event || '') })"
            />
          </VCol>
          ...
          <VCol cols="12" md="4">
            <AppSelect
              ...
              label="Field type"
              :items="['text', 'number', 'date', 'select', 'boolean', 'textarea']"
              @update:model-value="updateCustomField(index, { type: $event as DealCustomFieldType })"
            />
          </VCol>
